// GLOBAL VARIABLES
 let masterBank = []; let questions = []; let state = []; let currentQ = 0; let totalSeconds = 0; let sectionalSeconds = 0; let timeSpentGlobal = 0; let timerId;
 let currentMode = 'home'; let examType = 'mock'; let defaultLang = 'hi'; let currentLang = 'hi'; let forcedEn = false; let isCalcDrill = false; let isPaused = false;
 ...
 let userSprintTime = parseInt(localStorage.getItem('vu_sprint_time')) || 15; let userSprintQs = parseInt(localStorage.getItem('vu_sprint_qs')) || 25;
 let userMockGlobalTime = parseInt(localStorage.getItem('vu_mock_global_time')) || 60; let userMockGlobalQs = parseInt(localStorage.getItem('vu_mock_global_qs')) || 100;
 let userMockSplitMode = localStorage.getItem('vu_mock_split_mode') || 'auto'; let userMockSecData = JSON.parse(localStorage.getItem('vu_mock_sec_data') || '{}');

 let reviewFilteredIndices = []; let activeReviewStatuses = []; let isReviewSluggish = false;
 let revFilters = []; let revCurrentPage = 1; let revSubjectState = 'all'; let revTopicState = 'all'; let activeSliderUid = null;
 let mockBoundaries = []; let activeSectionIndex = 0; 

 const STORAGE_SEEN = 'VU_ENGINE_SEEN_UIDS'; const STORAGE_WRONG = 'VU_ENGINE_WRONG_UIDS'; const STORAGE_SESSION = 'VU_ACTIVE_SESSION'; const STORAGE_IMAGES = 'VU_IMAGE_SOLUTIONS'; const STORAGE_GITHUB_TOKEN = 'VU_GITHUB_TOKEN'; const STORAGE_TIME = 'VU_ENGINE_TIME_MAP';
 
 // --- NAVIGATION & RECOVERY ---
 function goHomeAlert() {
     if (typeof closeSlider === 'function') closeSlider();
     if(currentMode === 'test' || currentMode === 'practice') { if(!confirm("Exit to Dashboard? Your progress will be permanently lost.")) return; }
     ...
     screens.forEach(s => { const el = document.getElementById(s); if(el) el.style.setProperty('display', (s === 'home-screen') ? 'flex' : 'none', 'important'); });
     questions = []; state = [];
 }
 ...
 function openSettings() {
     document.getElementById('set-sprint-time').value = userSprintTime; document.getElementById('set-sprint-qs').value = userSprintQs; document.getElementById('set-mock-global-time').value = userMockGlobalTime; document.getElementById('set-mock-global-qs').value = userMockGlobalQs;
     const modalContainer = document.getElementById('modal-subject-toggles'); modalContainer.innerHTML = '';
     const subjects = [...new Set(masterBank.map(q => q.subject).filter(Boolean))];
     subjects.forEach(sub => { const btn = document.createElement('button'); btn.className = 'sub-toggle-btn' + (globalActiveSubjects.includes(sub) ? ' active' : ''); btn.innerText = sub; btn.onclick = () => { toggleGlobalSubject(sub, btn); renderSectionalDistribution(); }; modalContainer.appendChild(btn); });
     const calcContainer = document.getElementById('modal-calc-toggles'); calcContainer.innerHTML = '';
     defaultCalcTopics.forEach(sub => { const btn = document.createElement('button'); btn.className = 'sub-toggle-btn' + (globalCalcSubjects.includes(sub) ? ' active' : ''); btn.innerText = sub; btn.onclick = () => { toggleCalcSubject(sub, btn); }; calcContainer.appendChild(btn); });
     setSplitMode(userMockSplitMode); document.getElementById('settings-overlay').style.display = 'flex';
 }
 function closeSettings() { document.getElementById('settings-overlay').style.display = 'none'; }
 function switchSettingsTab(tabName) { document.querySelectorAll('.master-tab').forEach(t => t.classList.remove('active')); document.querySelectorAll('.settings-pane').forEach(p => p.classList.remove('active')); document.getElementById(`tab-${tabName}`).classList.add('active'); document.getElementById(`pane-${tabName}`).classList.add('active'); }
 function toggleCalcSubject(sub, btn) { const idx = globalCalcSubjects.indexOf(sub); if (idx > -1) { if (globalCalcSubjects.length === 1) { alert("Keep at least one active!"); return; } globalCalcSubjects.splice(idx, 1); btn.classList.remove('active'); } else { globalCalcSubjects.push(sub); btn.classList.add('active'); } localStorage.setItem('vu_calc_subjects', JSON.stringify(globalCalcSubjects)); }
 function setSplitMode(mode) { userMockSplitMode = mode; const isAuto = mode === 'auto'; document.getElementById('btn-auto-split').style.background = isAuto ? 'var(--cbt-blue)' : '#eee'; document.getElementById('btn-auto-split').style.color = isAuto ? 'white' : '#555'; document.getElementById('btn-custom-split').style.background = !isAuto ? 'var(--cbt-blue)' : '#eee'; document.getElementById('btn-custom-split').style.color = !isAuto ? 'white' : '#555'; if(isAuto) syncFromGlobal(); else renderSectionalDistribution(); }
 function syncFromGlobal() { if(userMockSplitMode === 'auto' && globalActiveSubjects.length > 0) { let totalTime = parseInt(document.getElementById('set-mock-global-time').value) || 0; let timePerSec = Math.floor(totalTime / globalActiveSubjects.length); globalActiveSubjects.forEach(sub => { if(!userMockSecData[sub]) userMockSecData[sub] = { qs: 25 }; userMockSecData[sub].time = timePerSec; }); } renderSectionalDistribution(); }
 function syncFromSectional() { let totalTime = 0, totalQs = 0; globalActiveSubjects.forEach(sub => { let timeInput = document.getElementById(`sec-time-${sub}`), qsInput = document.getElementById(`sec-qs-${sub}`); if(timeInput) totalTime += parseInt(timeInput.value) || 0; if(qsInput) totalQs += parseInt(qsInput.value) || 0; userMockSecData[sub] = { time: timeInput ? parseInt(timeInput.value) || 0 : 0, qs: qsInput ? parseInt(qsInput.value) || 0 : 0 }; }); if (userMockSplitMode === 'custom') document.getElementById('set-mock-global-time').value = totalTime; document.getElementById('set-mock-global-qs').value = totalQs; }
 function renderSectionalDistribution() { let container = document.getElementById('sectional-distribution-container'); container.innerHTML = ''; if (globalActiveSubjects.length === 0) { container.innerHTML = '<div style="text-align:center; padding:10px; color:#888;">No active subjects.</div>'; return; } globalActiveSubjects.forEach(sub => { if(!userMockSecData[sub]) { userMockSecData[sub] = { time: 15, qs: 25 }; } let isAuto = userMockSplitMode === 'auto'; let row = document.createElement('div'); row.className = 'settings-row'; row.style.cssText = 'background:#fcfcfc; padding:8px 12px; border-radius:6px; border:1px solid #eee;'; let tInputHTML = isAuto ? `<input type="number" id="sec-time-${sub}" class="settings-input" value="${userMockSecData[sub].time}" disabled style="background:#e9ecef;">` : `<input type="number" id="sec-time-${sub}" class="settings-input" value="${userMockSecData[sub].time}" oninput="syncFromSectional()">`; row.innerHTML = `<div style="font-weight:bold; color:var(--cbt-blue); flex:1;">${sub}</div><div class="aligned-input-grid">${tInputHTML}<span style="font-size:0.8rem; color:#888;">Min</span><input type="number" id="sec-qs-${sub}" class="settings-input" value="${userMockSecData[sub].qs}" oninput="syncFromSectional()"><span style="font-size:0.8rem; color:#888;">Qs</span></div>`; container.appendChild(row); }); }
 function saveSettings() { userSprintTime = parseInt(document.getElementById('set-sprint-time').value) || 15; userSprintQs = parseInt(document.getElementById('set-sprint-qs').value) || 25; userMockGlobalTime = parseInt(document.getElementById('set-mock-global-time').value) || 60; userMockGlobalQs = parseInt(document.getElementById('set-mock-global-qs').value) || 100; syncFromSectional(); localStorage.setItem('vu_sprint_time', userSprintTime); localStorage.setItem('vu_sprint_qs', userSprintQs); localStorage.setItem('vu_mock_global_time', userMockGlobalTime); localStorage.setItem('vu_mock_global_qs', userMockGlobalQs); localStorage.setItem('vu_mock_split_mode', userMockSplitMode); localStorage.setItem('vu_mock_sec_data', JSON.stringify(userMockSecData)); updateDashboardDropdowns(); closeSettings(); }

 // --- CLOUD SYNC ---
 const GITHUB_SYNC_FILE = 'user_sync.json'; const GITHUB_API_BASE = 'https://api.github.com/repos/the-vu-bot/exam-engine-vu/contents/';
 async function pushToCloud() { let token = localStorage.getItem(STORAGE_GITHUB_TOKEN); if (!token) { token = prompt("Enter GitHub PAT:"); if (!token) return; localStorage.setItem(STORAGE_GITHUB_TOKEN, token); } const payload = { seen: JSON.parse(localStorage.getItem(STORAGE_SEEN) || '[]'), wrongMap: JSON.parse(localStorage.getItem(STORAGE_WRONG) || '{}'), images: JSON.parse(localStorage.getItem(STORAGE_IMAGES) || '{}'), timeMap: JSON.parse(localStorage.getItem(STORAGE_TIME) || '{}'), settings: { subjects: globalActiveSubjects, calcSubjects: globalCalcSubjects, sprintTime: userSprintTime, sprintQs: userSprintQs, mockGlobalTime: userMockGlobalTime, mockGlobalQs: userMockGlobalQs, mockSplitMode: userMockSplitMode, mockSecData: userMockSecData } }; const btn = document.getElementById('cloud-push-btn'); const oT = btn.innerHTML; btn.innerHTML = "Syncing... ⏳"; try { const apiUrl = GITHUB_API_BASE + GITHUB_SYNC_FILE; let sha = null; const checkRes = await fetch(apiUrl, { headers: { 'Authorization': `Bearer ${token}` } }); if (checkRes.ok) { sha = (await checkRes.json()).sha; } const contentB64 = btoa(unescape(encodeURIComponent(JSON.stringify(payload, null, 2)))); const res = await fetch(apiUrl, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ message: `Vault Sync: ${new Date().toISOString()}`, content: contentB64, sha: sha }) }); if (res.ok) { btn.innerHTML = "✅ Synced!"; setTimeout(() => btn.innerHTML = oT, 3000); } else { alert("Failed."); btn.innerHTML = oT; } } catch (e) { console.error(e); btn.innerHTML = oT; } }
 async function pullFromCloud() { let token = localStorage.getItem(STORAGE_GITHUB_TOKEN); if (!token) { token = prompt("Enter GitHub PAT:"); if (!token) return; localStorage.setItem(STORAGE_GITHUB_TOKEN, token); } const btn = document.getElementById('cloud-pull-btn'); const oT = btn.innerHTML; btn.innerHTML = "Pulling... ⏳"; try { const apiUrl = GITHUB_API_BASE + GITHUB_SYNC_FILE; const res = await fetch(apiUrl, { headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/vnd.github.v3+json' } }); if (res.ok) { const data = JSON.parse(decodeURIComponent(escape(atob((await res.json()).content)))); if (data.seen) localStorage.setItem(STORAGE_SEEN, JSON.stringify(data.seen)); if (data.wrongMap) localStorage.setItem(STORAGE_WRONG, JSON.stringify(data.wrongMap)); if (data.images) localStorage.setItem(STORAGE_IMAGES, JSON.stringify(data.images)); if (data.timeMap) { let localTM = JSON.parse(localStorage.getItem(STORAGE_TIME) || '{}'); for(let uid in data.timeMap) { localTM[uid] = Math.max(localTM[uid] || 0, data.timeMap[uid]); } localStorage.setItem(STORAGE_TIME, JSON.stringify(localTM)); } if (data.settings) { if (data.settings.subjects) { globalActiveSubjects = data.settings.subjects; localStorage.setItem('vu_global_subjects', JSON.stringify(globalActiveSubjects)); } if (data.settings.calcSubjects) { globalCalcSubjects = data.settings.calcSubjects; localStorage.setItem('vu_calc_subjects', JSON.stringify(globalCalcSubjects)); } if (data.settings.sprintTime !== undefined) { userSprintTime = data.settings.sprintTime; localStorage.setItem('vu_sprint_time', userSprintTime); } if (data.settings.sprintQs !== undefined) { userSprintQs = data.settings.sprintQs; localStorage.setItem('vu_sprint_qs', userSprintQs); } if (data.settings.mockGlobalTime !== undefined) { userMockGlobalTime = data.settings.mockGlobalTime; localStorage.setItem('vu_mock_global_time', userMockGlobalTime); } if (data.settings.mockGlobalQs !== undefined) { userMockGlobalQs = data.settings.mockGlobalQs; localStorage.setItem('vu_mock_global_qs', userMockGlobalQs); } if (data.settings.mockSplitMode) { userMockSplitMode = data.settings.mockSplitMode; localStorage.setItem('vu_mock_split_mode', userMockSplitMode); } if (data.settings.mockSecData) { userMockSecData = data.settings.mockSecData; localStorage.setItem('vu_mock_sec_data', JSON.stringify(userMockSecData)); } updateDashboardDropdowns(); if(document.getElementById('settings-overlay').style.display === 'flex') openSettings(); } btn.innerHTML = "✅ Restored!"; setTimeout(() => btn.innerHTML = oT, 3000); } else { alert("Failed."); btn.innerHTML = oT; } } catch (e) { console.error(e); btn.innerHTML = oT; } }
 ...
 async function checkAuth() {
     ...
     if(savedKey) {
         try {
             ...
             if(res.ok) { masterBank = await res.json(); setupDashboard(); if(!restoreSession()) { goHomeAlert(); } return; }
         } catch(e) {}
     }
 ...
 async function verifyPin() {
     ...
     try {
         ...
         if (!res.ok) { alert("❌ File not found!"); btn.innerText = "Initialize Engine"; return; }
         masterBank = await res.json(); localStorage.setItem('vu_unlocked_key', input); setupDashboard(); goHomeAlert();
     } catch (e) { alert("Error connecting to server. Please refresh."); }
 ...
 function setupDashboard() {
     const dbStats = document.getElementById('db-stats'); if(dbStats) dbStats.innerText = `${masterBank.length} VU Patterns Loaded`;
     const subjects = [...new Set(masterBank.map(q => q.subject).filter(Boolean))];
 ...
 function toggleGlobalSubject(sub, btn) { const idx = globalActiveSubjects.indexOf(sub); if (idx > -1) { if (globalActiveSubjects.length === 1) { alert("Need at least one active subject."); return; } globalActiveSubjects.splice(idx, 1); btn.classList.remove('active'); } else { globalActiveSubjects.push(sub); btn.classList.add('active'); } localStorage.setItem('vu_global_subjects', JSON.stringify(globalActiveSubjects)); saveTestState(); updateDashboardDropdowns(); }
 function updateDashboardDropdowns() { let oHtml = `<option value="all">Mixed (Active Subjects)</option>`; globalActiveSubjects.forEach(sub => { oHtml += `<option value="${sub}">${sub}</option>`; }); if(document.getElementById('sprint-subject')) document.getElementById('sprint-subject').innerHTML = oHtml; if(document.getElementById('practice-subject')) document.getElementById('practice-subject').innerHTML = oHtml; }
 async function startCalculationMode() { const currentKey = localStorage.getItem('vu_unlocked_key'); const btn = document.getElementById('calc-btn'); btn.innerText = "Loading..."; try { const res = await fetch(currentKey + '_calc.json?t=' + new Date().getTime()); if (res.ok) { masterBank = await res.json(); startExam('practice', 'calc_drill', true); } else { alert("❌ Calc file missing."); } } catch (e) { alert("Error."); } btn.innerText = "START CALC DRILLS"; }
 ...
 // --- SESSION RESTORE ---
 function saveTestState() { if (document.getElementById('rev-subj-filter')) { revSubjectState = document.getElementById('rev-subj-filter').value; revTopicState = document.getElementById('rev-topic-filter').value; } const session = { questions, state, currentQ, totalSeconds, sectionalSeconds, activeSectionIndex, examType, defaultLang, currentLang, mockBoundaries, timeSpentGlobal, currentMode, forcedEn, isCalcDrill, revFilters, revCurrentPage, revSubjectState, revTopicState, globalActiveSubjects }; localStorage.setItem(STORAGE_SESSION, JSON.stringify(session)); }
 ...
 function startExam(type, subject, isCalcFlag = false) {
     ...
     document.getElementById('mobile-submit-btn').classList.remove('hide-for-review'); document.getElementById('review-filter-bar').style.display = 'none';
 ...
 function loadQuestion(index) {
     ...
     if(currentMode === 'review') {
         ...
         solCont.innerHTML = `<div class="solution-box" style="position:relative; padding-top:35px;"><div style="font-weight:bold; color:var(--cbt-blue); margin-bottom:15px;">Solution & Analysis</div><strong>VU Logic:</strong> ${q.logic || 'No text logic provided.'}<br><br><strong>Trap Alert:</strong> ${q.trap || 'None.'}</div>`;
 ...
 function loadNext() { if (currentMode === 'review' && reviewFilteredIndices.length > 0) { let idx = reviewFilteredIndices.indexOf(currentQ); if (idx !== -1 && idx < reviewFilteredIndices.length - 1) { loadQuestion(reviewFilteredIndices[idx + 1]); } return; } let tQ = currentQ + 1; if (examType === 'mock' && currentMode === 'test') { if(tQ > mockBoundaries[activeSectionIndex].end) { updatePaletteUI(); return; } } else if (tQ >= questions.length) { updatePaletteUI(); return; } loadQuestion(tQ); }
 function loadPrevious() { if (currentMode === 'review' && reviewFilteredIndices.length > 0) { let idx = reviewFilteredIndices.indexOf(currentQ); if (idx > 0) { loadQuestion(reviewFilteredIndices[idx - 1]); } return; } let tQ = currentQ - 1; if (examType === 'mock' && currentMode === 'test') { if(tQ < mockBoundaries[activeSectionIndex].start) return; } else if (tQ < 0) return; loadQuestion(tQ); }

 function startTestTimer() {
     clearInterval(timerId);
     timerId = setInterval(() => {
         if(isPaused) return; timeSpentGlobal++;
         if(currentMode === 'test' || currentMode === 'practice') {
             state[currentQ].timeTaken++;
             if(examType === 'mock') {
                 totalSeconds--; sectionalSeconds--;
                 document.getElementById('timer').innerText = formatTime(totalSeconds); document.getElementById('sec-timer').innerText = formatTime(sectionalSeconds);
             ...
             } else { document.getElementById('global-timer-label').innerText = 'Stopwatch'; document.getElementById('timer').innerText = formatTime(timeSpentGlobal); }
         }
 ...
 function buildPalette() {
     ...
     mockBoundaries.forEach((sec, bIndex) => {
         ...
         let lockStatus = '';
         if(examType === 'mock' && currentMode === 'test') { if (bIndex < activeSectionIndex) lockStatus = ' <span style="color:#888;">(Done)</span>'; else if (bIndex > activeSectionIndex) lockStatus = ' <span style="color:#d9534f;">(Locked)</span>'; else lockStatus = ' <span style="color:var(--cbt-green);">(Active)</span>'; }
 ...
 function updatePaletteUI() {
     ...
     questions.forEach((_, i) => {
         ...
         const btn = document.getElementById(`pal-${i}`); if(!btn) return; btn.className = 'pal-btn'; 
         if(currentMode === 'test' || currentMode === 'practice') {
             let isLocked = false; if(examType === 'mock') { let sB = mockBoundaries[activeSectionIndex]; if (i < sB.start || i > sB.end) isLocked = true; }
             if (isLocked) { btn.classList.add('status-locked'); } else { if(s === 'answered') btn.classList.add('status-answered'); else if(s === 'not-answered') btn.classList.add('status-not-answered'); else if(s === 'review') btn.classList.add('status-review'); if(i === currentQ) btn.classList.add('status-active'); }
         } else if (currentMode === 'review') {
             if (!reviewFilteredIndices.includes(i)) { btn.classList.add('filtered-out'); } else { if(s === 'unvisited') {} else if(sel === null) btn.classList.add('rev-skipped'); else if (isC) btn.classList.add('rev-correct'); else btn.classList.add('rev-incorrect'); if(i === currentQ) btn.classList.add('status-active'); }
         }
     });
     if(currentMode === 'review') {
 ...
 function toggleMobilePalette() { const panel = document.getElementById('right-panel-id'); panel.classList.toggle('open'); const tText = document.getElementById('palette-toggle-text'); tText.innerText = panel.classList.contains('open') ? "▼ Hide Question Palette" : "▲ Show Question Palette"; }
 ...
 function submitExam() {
     ...
     const seen = JSON.parse(localStorage.getItem(STORAGE_SEEN) || '[]'); const wrongMap = JSON.parse(localStorage.getItem(STORAGE_WRONG) || '{}'); const timeMap = JSON.parse(localStorage.getItem(STORAGE_TIME) || '{}');
     state.forEach((s, i) => {
         const uid = questions[i].uid;
         if(uid) {
             if(!seen.includes(uid)) seen.push(uid);
             if(s.selected !== null) {
                 if(s.selected === questions[i].correct) { if(wrongMap[uid]) wrongMap[uid]--; } else { wrongMap[uid] = (wrongMap[uid] || 0) + 1; }
                 if(s.timeTaken > 0) { if(!timeMap[uid]) timeMap[uid] = s.timeTaken; else timeMap[uid] = Math.round((timeMap[uid] + s.timeTaken) / 2); }
             }
         }
     });
     localStorage.setItem(STORAGE_SEEN, JSON.stringify(seen.slice(-500))); localStorage.setItem(STORAGE_WRONG, JSON.stringify(wrongMap)); localStorage.setItem(STORAGE_TIME, JSON.stringify(timeMap));
 ...
 function copyInsights(btn) { const t = document.getElementById('insight-text'); t.select(); navigator.clipboard.writeText(t.value).then(() => { const o = btn.innerHTML; btn.innerHTML="✅ Copied!"; btn.classList.replace('text-fuchsia-400', 'text-emerald-400'); setTimeout(() => { btn.innerHTML=o; btn.classList.replace('text-emerald-400', 'text-fuchsia-400'); }, 2000); }); }
 ...
 // --- REVIEW FILTERS ---
 function setupReviewFilters() { activeReviewStatuses = []; isReviewSluggish = false; ['correct', 'wrong', 'skipped', 'sluggish'].forEach(f => { let el = document.getElementById(`rev-filt-${f}`); if(el) el.classList.remove('active'); }); const sSel = document.getElementById('review-subj-filter'); let subs = new Set(); questions.forEach(q => { if(q.subject) subs.add(q.subject); }); sSel.innerHTML = '<option value="all">All Subjects</option>'; Array.from(subs).sort().forEach(s => sSel.innerHTML += `<option value="${s}">${s}</option>`); updateReviewTopicDropdown(); applyReviewFilters(); }
 function updateReviewTopicDropdown() { const subj = document.getElementById('review-subj-filter').value; const tSel = document.getElementById('review-topic-filter'); let tops = new Set(); questions.forEach(q => { if ((subj === 'all' || q.subject === subj) && q.topic) tops.add(q.topic); }); tSel.innerHTML = '<option value="all">All Topics</option>'; Array.from(tops).sort().forEach(t => tSel.innerHTML += `<option value="${t}">${t}</option>`); }
 ...
 function applyReviewFilters() { const subj = document.getElementById('review-subj-filter').value; const topic = document.getElementById('review-topic-filter').value; reviewFilteredIndices = []; for (let i = 0; i < questions.length; i++) { const q = questions[i], s = state[i], sel = s.selected, isC = sel === q.correct; let qS = 'unvisited'; if(s.status !== 'unvisited' && sel === null) qS = 'skipped'; else if (sel !== null && isC) qS = 'correct'; else if (sel !== null && !isC) qS = 'wrong'; if ((subj === 'all' || q.subject === subj) && (topic === 'all' || q.topic === topic) && (activeReviewStatuses.length === 0 || activeReviewStatuses.includes(qS))) { reviewFilteredIndices.push(i); } } if (isReviewSluggish) { reviewFilteredIndices.sort((a, b) => state[b].timeTaken - state[a].timeTaken); } else { reviewFilteredIndices.sort((a, b) => a - b); } updatePaletteUI(); if (reviewFilteredIndices.length > 0) { if (!reviewFilteredIndices.includes(currentQ)) { loadQuestion(reviewFilteredIndices[0]); } else { loadQuestion(currentQ); } } else { document.getElementById('q-text').innerHTML = "<div style='text-align:center; color:#888; margin-top:20px;'>No questions match filters.</div>"; document.getElementById('options-container').innerHTML = ''; document.getElementById('solution-container').style.display = 'none'; document.getElementById('q-num-display').innerText = '-'; document.getElementById('q-status-badge').innerHTML = ''; } }
 ...
 function renderRevisionFeed() {
     ...
     if (subject !== 'all') pool = pool.filter(q => q.subject === subject); if (topic !== 'all') pool = pool.filter(q => q.topic === topic);
     ...
     if (pagedPool.length === 0) { container.innerHTML = `<div style="text-align:center; padding:30px; color:#888;">No questions match.</div>`; } 
 ...
 function fetchPrivateImage(url, id) { 
     const el = document.getElementById(id); 
     if(el) { el.src = url; el.style.display = 'block'; if(document.getElementById(id+'-wrapper')) document.getElementById(id+'-wrapper').style.display='none'; } 
 }
